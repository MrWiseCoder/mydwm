...
static void focusstack2(const Arg *arg);
// -----------------------------------------
...
static void pushstack2(const Arg *arg);
// %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
//        +++ Xresources +++
// %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
int drawstatusbar(Monitor *m, int bh, char *stext) {
  ...
  // --------------- me ---------------
  drw->scheme[ColFg] = scheme[Ado6][ColFg];
  drw->scheme[ColBg] = scheme[Ado6][ColBg];
  // ---------------------------------------
    ...
    if (text[i] == '^' && !isCode) {
      ...
      while (text[++i] != '^') {
        if (text[i] == 'c') {
          ...
          memcpy(buf2, (char *)text + i + 1, 4);
          buf2[4] = '\0';
          if (strcmp("Ado1", buf2) == 0) {
            drw->scheme[ColFg] = scheme[Ado1][ColFg];
            drw->scheme[ColBg] = scheme[Ado1][ColBg];
          } else if (strcmp("Ado2", buf2) == 0){
            drw->scheme[ColFg] = scheme[Ado2][ColFg];
            drw->scheme[ColBg] = scheme[Ado2][ColBg];
          } else if (strcmp("Ado3", buf2) == 0){
            drw->scheme[ColFg] = scheme[Ado3][ColFg];
            drw->scheme[ColBg] = scheme[Ado3][ColBg];
          } else if (strcmp("Ado4", buf2) == 0){
            drw->scheme[ColFg] = scheme[Ado4][ColFg];
            drw->scheme[ColBg] = scheme[Ado4][ColBg];
          } else if (strcmp("Ado5", buf2) == 0){
            drw->scheme[ColFg] = scheme[Ado5][ColFg];
            drw->scheme[ColBg] = scheme[Ado5][ColBg];
          } else if (strcmp("Ado6", buf2) == 0){
            drw->scheme[ColFg] = scheme[Ado6][ColFg];
            drw->scheme[ColBg] = scheme[Ado6][ColBg];
          } else if (strcmp("Ado7", buf2) == 0){
            drw->scheme[ColFg] = scheme[Ado7][ColFg];
            drw->scheme[ColBg] = scheme[Ado7][ColBg];
          } else if (strcmp("Ado8", buf2) == 0){
            drw->scheme[ColFg] = scheme[Ado8][ColFg];
            drw->scheme[ColBg] = scheme[Ado8][ColBg];
          } else if (strcmp("Ado9", buf2) == 0){
            drw->scheme[ColFg] = scheme[Ado9][ColFg];
            drw->scheme[ColBg] = scheme[Ado9][ColBg];
          } else if (strcmp("Ado0", buf2) == 0){
            drw->scheme[ColFg] = scheme[Ado0][ColFg];
            drw->scheme[ColBg] = scheme[Ado0][ColBg];
          }
          i += 4;
        } else if (text[i] == 'd') {
          drw->scheme[ColFg] = scheme[SchemeNorm][ColFg];
  ...
  // --------------- me ---------------
  /** drw_setscheme(drw, scheme[AdoStatusColor]); */
  // --------------- chad ---------------
...
void drawbar(Monitor *m) {
   ...
   * +------------------------+ */
  // --------------- me ---------------
  /** XSetForeground(drw->dpy, drw->gc, scheme[AdoSpecColor][ColBg].pixel); */
  // --------------- chad ---------------
  ...
  if (m == selmon) { /* status is only drawn on selected monitor */
    // --------------- me ---------------
    sw = mw - drawstatusbar(m, bh, stext);
    // --------------- chad ---------------
  ...
  if ((w = mw + m->gappov * 2 - sw - stw - x) > bh_n) {
    if (m->sel) {
      // --------------- me ---------------
      drw_setscheme(drw, scheme[m == selmon ? AdoSelColor : AdoNormColor]);
      // --------------- chad ---------------
      ...
      // --------------- me ---------------
      drw_setscheme(drw, scheme[AdoNormColor]);
      // --------------- chad ---------------
      /** drw_setscheme(drw, scheme[SchemeNorm]); */
      // ---------------------------------------
...
// %%%%%%%%%%%%%%%%%%%%%%%%%%%%
// Push and Focus Stack
// %%%%%%%%%%%%%%%%%%%%%%%%%%%%
void
pushstack2(const Arg *arg) {
	int i = stackpos(arg);
